...
class Audio {
  ...
  async record() {
    ...
    await mediaRecorder.addEventListener("stop", () => {
      this._data.file = new File(
        [voiceBlob],
        `${this._data.name}.${voiceBlob.type.slice(6, 10)}`
      );
    ...
    let audio = this.element.firstChild,
      ...
      inputName = this.DOMCreate('input', [this._CSS.input, 'audio__record-name'], {
        type: 'text',
        value: this._data.name
      }),
      play = this.DOMCreate('button', [this._CSS.button, 'audio__record-start'], {
        id: 'record-play',
        type: 'button',
        innerHTML: `${ObjectSVG.record} Record`
      }),
      stop = this.DOMCreate('button', [this._CSS.button, 'audio__record-stop'], {
        id: 'record-stop',
        type: 'button',
        innerHTML: `${ObjectSVG.stop} Stop`
      });

    record.append(inputName)
  ...
  createRecordListening() {
    let wrapper = this.DOMCreate('div', 'audio__record-wrapper'),
      save = this.DOMCreate('button', 'audio__record-save', {
        type: 'button',
        innerHTML: ObjectSVG.save
      }),
      clear = this.DOMCreate('button', 'audio__record-clear', {
        type: 'button',
